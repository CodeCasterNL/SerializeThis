...
The experimental instance has its own file history, so click "Continue without code" on the startup screen, click `File -> Open -> File...` and select the file you wish to test, for example the file `src\CodeCaster.SerializeThis\JsonTestClasses.cs` in this repository. On later runs of the experimental instance, you can then click `File -> Recent Files -> ...JsonTestClasses.cs` so you won't have to browse to it again.
